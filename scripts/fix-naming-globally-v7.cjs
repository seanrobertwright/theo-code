...
function fixNaming(filePath) {
  ...
  // Improved regex to find underscored parameters anywhere in the parameter list
  // Matches: (_param: Type) OR (param1: Type, _param2: Type)
  const paramRegex = /[(,]\\s*(_+\\w+):/g;
  let match;
  while ((match = paramRegex.exec(content)) !== null) {
    const oldParam = match[1];
    const newParam = oldParam.replace(/^_+/, '');
    
    // Replace all occurrences of this parameter being declared
    const declarationRegex = new RegExp('([(,]\\s*)' + escapeRegExp(oldParam) + ':', 'g');
    content = content.replace(declarationRegex, '$1' + newParam + ':');
  ...
  if (changed) {
    ...
    console.log(`Global fixed naming in ${path.basename(filePath)}`);
  }
}
